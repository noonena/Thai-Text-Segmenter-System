...
import { TrendingUp, AlertCircle, BarChart3, Info } from "lucide-react";
import {
...
function StatisticsPage() {
  ...
  if (error === "not_found" || !data) return (
    <div className="flex flex-col items-center justify-center h-48 gap-4">
      <p className="text-gray-600 text-sm">No evaluation results yet. Train the models to generate results.</p>
    </div>
  );
      ...
      <div className="px-8">
                    ...
                    {[
                      ...
                      <tr key={row.label} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 border-b border-gray-100 text-center">
                          <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${row.f >= 0.9 ? "bg-green-100 text-green-700" :
        ...
        {activeSection === "segmentation" && (
          <div className="space-y-6">
            ...
            {/* MTU & Syllable detail */}
            <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
              <h3 className="text-sm font-semibold text-gray-900 mb-4">Stage Comparison — Precision / Recall / F1</h3>
              <div className="overflow-x-auto mb-6">
                <table className="w-full text-sm border-collapse">
                  <thead>
                    <tr className="bg-gray-50 text-xs font-semibold text-gray-600">
                  ...
                  <tbody>
                    {[
                      ...
                      <tr key={row.label} className="hover:bg-gray-50">
                        <td className="px-4 py-2 border-b border-gray-100 font-semibold">{row.label}</td>
                        <td className="px-4 py-2 border-b border-gray-100 text-center">{pct(row.p)}%</td>
                        <td className="px-4 py-2 border-b border-gray-100 text-center">{pct(row.r)}%</td>
                        <td className="px-4 py-2 border-b border-gray-100 text-center">
                          <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${row.f >= 0.9 ? "bg-green-100 text-green-700" :
                            row.f >= 0.7 ? "bg-yellow-100 text-yellow-700" : "bg-red-100 text-red-700"}`}>
                            {pct(row.f)}%
                          </span>
                        </td>
                      </tr>
                  ...
                  <tbody>
                    {topPosTags.map(([tag, v]) => (
                      <tr key={tag} className="hover:bg-gray-50">
                        ...
                        <td className="px-4 py-2 border-b border-gray-100 text-center">
                          <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${v.f1 >= 0.9 ? "bg-green-100 text-green-700" :
