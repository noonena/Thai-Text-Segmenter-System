...
#     POST /api/process-html
#     {
#         "html_content": "<html><body><p>สวัสดีครับผมชื่อวิน</p></body></html>",
#         "filename": "test.html",
#         "settings": {
#             "tag": "span",
#             "cssClass": "no-wrap"
...
#     try:
#         # ✅ FIX 6: Parse HTML with explicit UTF-8 encoding
...
#             try:
#                 # ✅ FIX 7: Ensure text is UTF-8 before segmentation
#                 text_clean = text.strip()
                
#                 # Segment the text using complete pipeline (3 layers)
...
#                         # Wrap Thai words with word break opportunity
#                         if css_class:
#                             wrapped_parts.append(f'<wbr><{tag} class="{css_class}">{word}</{tag}>')
#                         else:
#                             wrapped_parts.append(f'<wbr><{tag}>{word}</{tag}>')
#                     else:
#                         # Keep non-Thai text as is (spaces, punctuation, English)
#                         wrapped_parts.append(word)
                
#                 result = ''.join(wrapped_parts)
                
#                 # ✅ FIX 8: Validate UTF-8 encoding of result
#                 result.encode('utf-8')
                
#                 return result
                
#             except Exception as e:
#                 print(f"❌ Error processing text node: {text[:50]}... Error: {e}")
#                 import traceback
#                 traceback.print_exc()
...
#             # Replace with new wrapped version if changed
#             if new_text != original_text:
#                 # ✅ FIX 9: Parse with UTF-8 encoding
#                 new_soup = BeautifulSoup(new_text, 'html.parser', from_encoding='utf-8')
#                 element.replace_with(new_soup)
#                 processed_count += 1
        
#         # Return processed HTML
#         # ✅ FIX 10: Ensure output is UTF-8
#         wrapped_html = str(soup)
        
#         # Verify encoding
#         try:
#             wrapped_html.encode('utf-8')
#         except UnicodeEncodeError as e:
#             raise HTTPException(
#                 status_code=500,
#                 detail=f"Output encoding error: {str(e)}"
#             )
        
#         print(f"✅ Successfully processed {processed_count} text nodes")
...
# In app.py, update the /api/process-html endpoint
...
# ✅ FIX 11: Add encoding test endpoint
@app.get("/api/test-encoding")
async def test_encoding():
    """Test endpoint to verify Thai encoding works correctly"""
    test_text = "สวัสดีครับผมชื่อวิน"
    
    try:
        # Test segmentation
        if segmenter is None:
            return {"error": "Segmenter not loaded"}
        ...
        return {
            ...
            "utf8_check": all(word.encode('utf-8') for word in words)
        }
    except Exception as e:
        import traceback
        return {
