...
class WordSegmenter:
    ...
    def _is_real_word(self, candidate: str) -> bool:
        """Return True if candidate is a genuine dictionary word (not a generated compound)."""
        return self.dictionary.contains(candidate) and candidate not in self.dictionary.compounds
    ...
    def _score_candidate_v2(
        ...
        which discourages over-segmentation. Real dictionary words get a
        higher base bonus than generated compounds. Multi-syllable words
        ...
        if self.dictionary.contains(candidate):
            ...
            # Real (non-generated) words get a stronger compound bonus to favour
            # joining syllables (e.g. ราย+งาน → รายงาน).
            # Generated bigram/trigram compounds keep the original bonus to avoid
            # over-merging (e.g. ประเทศ+จีน should stay separate).
            bonus_per_syl = 4.0 if self._is_real_word(candidate) else 3.5
            compound_bonus = (unit_length - 1) * bonus_per_syl
    ...
    def _repair_syllables(self, syllables: List[str]) -> List[str]:
        ...
        while i < len(result) - 1:
            ...
            if not self.dictionary.contains(curr) and len(nxt) >= 2:
                ...
                repaired_nxt  = nxt[1:]
                if self._is_real_word(repaired_curr) and self._is_real_word(repaired_nxt):
